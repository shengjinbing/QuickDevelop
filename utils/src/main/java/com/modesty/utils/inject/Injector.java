...
import com.modesty.utils.ArrayUtils;
import com.modesty.utils.CheckUtils;
...
public class Injector {
    ...
    private static final Class<?>[] HALT_CLASSES = {
            Activity.class,
            Fragment.class,
            View.class,
            Object.class,
    ...
    public static void inject(Object container, Object parent, boolean ignore) {
        ...
        while (!ArrayUtils.contains(HALT_CLASSES, clazz)) {
    ...
    private static void throwIdException(Field field, String info, int... ids) {
        ...
        if (!CheckUtils.isEmpty(ids)) {
            sb.append("[");
            for (int id : ids) {
                sb.append(id + "=0x" + Integer.toHexString(id));
                sb.append(",");
            }
            sb.append("]");
        }
