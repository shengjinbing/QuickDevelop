...
/**
 *  3DES是DES加密算法的一种模式，它使用3条64位的密钥对数据进行三次加密。数据加密标准（DES）是美国的
 *  一种由来已久的加密标准，它使用对称密钥加密法。3DES（即Triple DES）是DES向AES过渡的加密算法
 *  （1999年，NIST将3-DES指定为过渡的加密标准），是DES的一个更安全的变形。它以DES为基本模块，通过组合分组方法设计出分组加密算法。
 */
...
*/
     ...
     * @throws Exception
     */
    private static Key getRawKeyT(String key) throws Exception {
    ...
    public static String encrypt(String key, byte[] data) {
        try {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, getRawKeyT(key), iv);
    ...
    public static String decrypt(String key, byte[] data) {
        try {
            Cipher cipher = Cipher.getInstance(TRANSFORMATION);
            IvParameterSpec iv = new IvParameterSpec(IVPARAMETERSPEC.getBytes());
            cipher.init(Cipher.DECRYPT_MODE, getRawKeyT(key), iv);
