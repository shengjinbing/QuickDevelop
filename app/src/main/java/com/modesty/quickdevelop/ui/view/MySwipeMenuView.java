...
public class MySwipeMenuView extends ViewGroup {
    private static final String TAG = "SWIPEMENUVIEW_LOG";
    /**
     * 滑动删除功能的开关,默认开
     */
     ...
     * 存储的是当前正在展开的View
     */
    private static MySwipeMenuView mViewCache;
     ...
     * @param ios
     */
    public MySwipeMenuView setIos(boolean ios) {
        isIos = ios;
        return this;
    }
     ...
     * @return
     */
    public MySwipeMenuView setLeftSwipe(boolean leftSwipe) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childView.getVisibility() != GONE) {
                ...
                if (i == 0) {
                    //第一个布局是Left item，
                    mLeftMenuWidths += childView.getMeasuredWidth();
                } else if (i == 1) {
                    //从第二个开始才是ContentView
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childView.getVisibility() != GONE) {
                if (i == 0) {
                    ...
                    if (isRightSwipe) {
                        //右滑是否打开
                        childView.layout(getPaddingLeft() - childView.getMeasuredWidth(), getPaddingTop(), getPaddingLeft(), getPaddingTop() + childView.getMeasuredHeight());
                        left += getPaddingLeft();
                    }
                } else if (i == 1) {
                    //第1个子View是内容 宽度设置为全屏
                    childView.layout(left, getPaddingTop(), getPaddingLeft() + childView.getMeasuredWidth(), getPaddingTop() + childView.getMeasuredHeight());
            ...
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (isLeftSwipe && !isRightSwipe) {
                        //右滑打开
                        if (getScrollX() < 0) {
                            //getScrollX()的值：当view的左边缘在view内容的左边缘的右边时，mScrolly为正
                            scrollTo(0, 0);
                        }
                        if (getScrollX() > mLeftMenuWidths) {
                            scrollTo(mLeftMenuWidths, 0);
                        }
    ...
    public void smoothExpand() {
        //展开就加入ViewCache：
        mViewCache = MySwipeMenuView.this;
        ...
        if (getScrollX() < 0) {
            ...
            //左滑展开
            mExpandAnim = ValueAnimator.ofInt(getScrollX(),  mLeftMenuWidths );
        }
