...
/**
 * rank1.no-cache :无缓存指令，即每次请求直接从服务器获取。”Cache-Control”： “no-cache
 * rank2.max-age :代表缓存的有效时间，如果缓存只是用来和服务器做验证，可是设置更有效的”Cache-Control”：”max-age=0”。
 * rank3.only-if-cached :先使用用缓存的数据，如果客户端有缓存，会立即显示客户端的缓存，
